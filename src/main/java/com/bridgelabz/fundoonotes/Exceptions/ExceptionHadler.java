...
public class ExceptionHadler
{
	...
	public ResponseEntity<UserException> loginException(UserResponse response)
	{
		...
		return ResponseEntity.status(HttpStatus.NOT_ACCEPTABLE).body(new UserException(exp.getData(),exp.getStatus(),exp.getMessage()));
	}
